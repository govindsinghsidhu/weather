...
public interface WeatherReportValidator {
	...
	public BiFunction<Double, WeatherReport, WeatherReport> temeratureAlert =(temprature, weatherReport) ->{ if(WeatherReportValidator.temperatureCheck.test(temprature)) weatherReport.setTemperatureAlert(ApplicationConstants.USE_SUNSCREEN); return weatherReport;};
	public BiFunction<Double, WeatherReport, WeatherReport> windAlert =(windSpeed, weatherReport) ->{ if(WeatherReportValidator.windCheck.test(windSpeed)) weatherReport.setWindAlert(ApplicationConstants.TOO_WINDY); return weatherReport;};
	public BiFunction<String, WeatherReport, WeatherReport> rainAlert =(rainStatus, weatherReport) ->{ if(WeatherReportValidator.rainCheck.test(rainStatus)) weatherReport.setRainAlert(ApplicationConstants.CARRY_UMBRELLA); return weatherReport;};
